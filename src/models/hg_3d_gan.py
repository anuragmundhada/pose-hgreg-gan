...
class Hourglass3DGAN():
  ...
  def __init__(self, opt):
    ...
    self.jointTriplets = [[0,1,2],[3, 4, 5],[10,11,12],[13,14,15],[2, 6, 3],[12,8,13]]
    ...
    if opt.loadDis != 'none':
      self.netD = []
      for it, triplets in enumerate(self.jointTriplets):
        self.netD.append(torch.load(os.path.join(opt.loadDis, '_{}.pth'.format(it))).cuda())
    else:
      for it, triplets in enumerate(self.jointTriplets):
        self.netD.append(TripleJointDis().cuda())
    ...
    self.criterion_D = []
    for it, triplets in enumerate(self.jointTriplets):
      self.optimizer_D.append(torch.optim.Adam(self.netD[it].parameters(), opt.disLR))
      self.criterion_D.append(torch.nn.MSELoss().cuda())
  ...
  def backward_G(self):
    ...
    for t in range(batchSize):
      ...
      if s < ref.eps and s > - ref.eps: 
        ...
        self.real_pool.append(torch.cat((reg[t], xy[t].view(32)), 0).view(1,16, 3))
      else:
        # GAN loss for unsupervised data
        l_GAN = 0
        fake_pose = torch.cat((reg[t].view(1,16, 1), xy[t].view(1, 16, 2)), 2)
        for it, triplets in enumerate(self.jointTriplets):
          n_tot += 1
          indices = torch.LongTensor(triplets).cuda()
          triplet_pose = fake_pose[:, indices, :].view(1, 3*indices.size(0))
          dis_fake = self.netD[it](triplet_pose)
          if dis_fake.data[0][0] > 0.5:
            # print(dis_fake.data[0][0])
            n_real+=1

          l_GAN += self.criterion_D[it](dis_fake, self.create_dis_label(dis_fake, True))
  ...
  def backward_D(self):
    if len(self.real_pool) > 0 and len(self.fake_pool) > 0:
      ...
      num_fake = len(self.fake_pool)
      real_poses = torch.cat(self.real_pool, 0).detach()
      fake_poses = torch.cat(self.fake_pool, 0).detach()
      loss_D_fake = 0
      loss_D_real = 0


      # Adding noise to real poses to prevent overfitting
      if self.noise_real > 0:
        noise = torch.autograd.Variable(torch.randn(real_poses.size()).cuda() * self.noise_real)
        real_poses += noise
      
      # Iterating over joint triplets
      for it, triplets in enumerate(self.jointTriplets):
        # Real
        indices = torch.LongTensor(triplets).cuda()
        triplet_poses_real = real_poses[:, indices, :].view(num_real, 9)
        pred_real = self.netD[it](triplet_poses_real)
        loss_D_real += self.criterion_D[it](pred_real, self.create_dis_label(pred_real, True))

        # Fake
        triplet_poses_fake = fake_poses[:, indices, :].view(num_fake, 9)
        pred_fake = self.netD[it](triplet_poses_fake)
        loss_D_fake += self.criterion_D[it](pred_fake, self.create_dis_label(pred_fake, False))
  ...
  def optimize_parameters(self):
      ...
      if self.isTrain:
        ...
        for it, triplets in enumerate(self.jointTriplets):
          self.optimizer_D[it].zero_grad()
          self.optimizer_D[it].step()
